 ...
 */
export async function markAuthCodeConsumed(jti: string, db: Pool | PoolClient = getPool()): Promise<boolean> {
  const { rowCount } = await db.query('INSERT INTO consumed_auth_codes (jti) VALUES ($1) ON CONFLICT DO NOTHING', [
    jti,
  ]);
  return (rowCount ?? 0) > 0;
}
