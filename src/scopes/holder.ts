...
export const holderHandler: ScopeHandler = {
  async verify(ctx, params) {
    ...
    try {
      ...
      console.warn(`On-chain holder check failed for ${ctx.coldkey} on subnet ${params.netuid}:`, err);
    }
    return await checkBalanceViaTaostatsApi(ctx.coldkey!, params.netuid, minAmount);
  },
};

async function checkBalanceViaTaostatsApi(address: string, netuid: number, minAmount: bigint): Promise<boolean> {
  try {
    const result = await taostatsGet<AlphaBalanceEntry>(`/api/dtao/stake_balance/latest/v1`, {
      coldkey: address,
